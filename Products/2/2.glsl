...
void rotCam(){
  ...
  C_up=rotate3D(90.* 0.5 * PI / 180.0,vec3(0,0,1.))*C_dir;
}
...
void main(){
  ...
  //L_pos+=vec3(40.*cos(time),5,40.*sin(time));
  vec3 color;
  if(abs(dist) < 0.001){
      float l=mod(floor(length(D_pos)),12.)/12.;
      vec3 normal = genNormal(D_pos);
      float diff = L_int*clamp(dot(normalize(D_pos-L_pos), normal), 0.1, 1.0)/length(D_pos-L_pos);
